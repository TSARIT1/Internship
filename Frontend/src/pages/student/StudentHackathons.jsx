...
const StudentHackathons = () => {
    ...
    const handleRegister = async (hackathon) => {
        ...
        if (confirm(`Are you sure you want to register for ${hackathon.title}?`)) {
            try {
                if (!hackathon.id) {
                    alert("Error: Hackathon ID is missing.");
                    return;
                }
                const response = await registerForHackathon(hackathon.id, student.id);
                if (response.success) {
                    alert(`Successfully registered for ${hackathon.title}!`);
                    loadHackathons(); // Refresh list to update status if I add that logic later
                } else {
                    alert(`Registration failed: ${response.message || response.error?.response?.data || "Unknown error"}`);
                }
            } catch (error) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {hackathons.length > 0 ? (
                        hackathons.map((hackathon, index) => {
                            if (!hackathon.id) {
                                console.warn(`Hackathon at index ${index} is missing an ID: ${JSON.stringify(hackathon)}`);
                            }
                            return (
                                <div key={hackathon.id || `hackathon-${index}`} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-lg transition-all group hover:border-emerald-200">
